...
const WaitingScreen = () => {
      ...
      const fetchOrder = async () => {
        try {
          ...
          const response = await fetch(
            `https://smart-server-stage-database-default-rtdb.firebaseio.com/history.json?orgId=${orgId}`
          );
          ...
          const fetchedOrder = ordersArray.find(
          ...
          setOrder({
            ...fetchedOrder,
            displayOrderId: fetchedOrder.id || orderId,
          });
        } catch (error) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      default:
        ...
        return (
          <QuestionOutlined style={{ fontSize: "48px", color: "#8c8c8c" }} />
        );
    }
  };
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      const response = await fetch(
        "https://smart-server-stage-database-default-rtdb.firebaseio.com/feedback.json",
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      >
        <Switch
          ...
          style={{
            backgroundColor: soundEnabled ? "#52c41a" : "#ff4d4f",
            borderColor: soundEnabled ? "#52c41a" : "#ff4d4f",
          }}
        />
        <Text style={{ marginLeft: "8px", color: "#343a40" }}>
          Sound Notifications
        </Text>
      </div>
    ...
    `}
        ...
        <Title level={4} style={{ marginTop: "16px", color: "#343a40" }}>
          {order.statusMessage}
        </Title>
        <Text type="secondary" style={{ color: "#6c757d" }}>
      ...
      >
        ...
        <p>
          We hope you enjoyed your meal. Please provide your feedback below:
        </p>
        <Input.TextArea

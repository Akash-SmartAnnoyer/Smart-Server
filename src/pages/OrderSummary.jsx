...
import FoodLoader from "../components/giftLoader";
...
function OrderSummary() {
  ...
  useEffect(() => {
    ...
    setSuggestedItems([
      ...
      },
      {
        ...
        quote: "Add a zesty touch!",
      },
      {
        name: "Onions",
        image:
          "https://cdn3.didevelop.com/public/cdn/533_85eeabd966a7facca7863150dea6c11a.jpg",
        quote: "Enhance the flavor!",
      },
    ]);
  ...
  const { total: calculatedTotal, breakdown } = calculateCharges(
    subtotal,
    enabledCharges
  );
  ...
  const handlePayClick = async () => {
    try {
      ...
      if (distance > MAX_DISTANCE_KM) {
        showErrorModal(
          <div style={{ textAlign: "center" }}>
            ...
            <p>
              You appear to be {distance.toFixed(2)}km away from{" "}
              {restaurant.name}.
            </p>
      ...
      if (isNaN(tableNum) || tableNum < 1 || tableNum > seatingCapacity) {
        showErrorModal(
          `Please enter a valid table number between 1 and ${seatingCapacity}`
        );
        return;
      }
      ...
      ws.current.onopen = () => {
        ws.current.send(
          JSON.stringify({ type: "newOrder", order: orderDetails })
        );
      };
  ...
  return (
    <div
      ...
      style={{ marginTop: "145px", marginBottom: "200px" }}
    >
    ...
    >
      ...
      >
        <div
          style={{ textAlign: "center", color: "#ff6347", fontSize: "1.2em" }}
        >
          🔥 Tell us your flavor! Extra spicy, less salt, saucy, or dry – your
          call! 🌶️👌
        </div>
        <p
          style={{
        ...
        >
          {suggestedItems.map((item) => (
            ...
            >
              ...
              />
              <div>
                <p style={{ margin: 0, fontWeight: "bold", color: "#ff6347" }}>
                  {item.name}
                </p>

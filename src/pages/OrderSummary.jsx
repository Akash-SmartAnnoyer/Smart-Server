...
import { CoffeeOutlined, SmileOutlined,ExclamationCircleOutlined, PlusCircleOutlined, CheckOutlined, Phone, ShieldCheck, PhoneOutlined } from '@ant-design/icons';
...
useEffect(() => {
  ...
  const handlePayClick = async () => {
    if (restaurantDetails?.phoneRequired && !phoneNumber) {
  ...
  const PhoneModal = () => (
    <Modal
      ...
      onCancel={() => {
        if (!restaurantDetails?.phoneMandatory) {
          setIsPhoneModalVisible(false);
        }
      }}
      footer={null}
      closable={!restaurantDetails?.phoneMandatory}
      maskClosable={!restaurantDetails?.phoneMandatory}
    ...
    >
      <div style={{
        ...
        <p style={{
          ...
          {restaurantDetails?.phoneMandatory 
            ? "Please provide your phone number to complete the order"
            : "Add your phone number for order updates (optional)"}
        </p>

        <div style={{
          position: 'relative',
          marginBottom: phoneError ? '30px' : '20px'
        }}>
          <Input
            prefix={<PhoneOutlined size={18} color="#FF0000" />}
            value={phoneNumber}
            onChange={(e) => {
